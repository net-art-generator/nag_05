...
sub startPage {
	my $title=(@_)? "NAG :: @_" : "NAG :: Net.Art Generator";
	return "<html xmlns='http://www.w3.org/1999/xhtml' lang='en-US'>
...
sub inputForm {
	...
	my @wid=(400,600,800,1000);
	my @im=(2,4,6,8);
	my %ext=('jpg'=>'JPEG :: Jpeg Image', 'gif'=>'GIF :: Gif Image', 'png'=>'PNG :: Portable Network Graphics Image');
...
sub rowTabs {
	...
	for (my $i=3;$i<@_;$i+=2) {
		...
		if ($_[$i] && $_[$i+1]) { $out .= "<a \nhref='" . $_[$i+1] . "'>"; }
		if ($_[1] && $_[$i]) { $out .= "<font color='$attr{'fontcolor'}'>"; }
		if ($_[$i]) { $out .= "<b>$_[$i]</b>"; }
		if ($_[1] && $_[$i]) { $out .= "</font>"; }
		if ($_[$i] && $_[$i+1]) { $out .= "</a>"; }
		$out .= "&nbsp;</td>";
